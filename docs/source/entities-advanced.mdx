...
### Understanding resolver consistency

If multiple subgraphs can resolve a field, queries may return that field from any of those subgraphs, depending on the query plan for a given shape of query. If those different subgraphs behave differently when resolving that field, your clients could end up with inconsistent results across different operations for that field.

This potential inconsistency is especially important to keep in mind when making changes to an _existing_ resolver. Until the resolvers on all possible subgraphs that return that field are aligned, clients could observe inconsistent results.

The two most common inconsistent behaviors to avoid for across subgraphs are:

* Returning a different default value for the same field
